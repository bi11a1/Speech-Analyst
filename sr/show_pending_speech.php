<?php
	...
	if(!isset($_GET['speech_no'])){
		...
		$query = "SELECT user_id, title, summary, language, category, location, upload_time, file_name FROM speech
					WHERE speech_no = ?";
		if($stmt = $con->prepare($query)){
			$stmt->bind_param("i", $speech_no);
			$stmt->execute();
			$stmt->bind_result($user_id, $title, $summary, $language, $category, $location, $upload_time, $file_name);
			if(!($stmt->fetch())){
...
<html>
<head>
	<title>Speech Repository | Show Speech</title>
		...
		<div class="col-8 home-div" style="background-image: linear-gradient(to bottom right, #eafaff, #ffffff);">
			<div class="welcome-div">
				...
				<div class="speech-details">
					...
					<center>
					<form action="accept_pending_speech.php?speech_no=<?php echo $speech_no; ?>" method="post">
	                    <input type="submit" id="login_btn" value="Accept" name="submit" style="padding: 5px;" title="Accept speech?">
	                </form>
	            	</center>
				</div>
			</div>
